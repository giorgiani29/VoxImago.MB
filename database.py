...
class FileIndexer:
    ...
    def _create_tables(self):
        ...
        ''')
        self.cursor.execute(
            'CREATE VIRTUAL TABLE IF NOT EXISTS search_index USING fts5(name, description, file_id UNINDEXED, source UNINDEXED, tokenize="trigram")')
        self.cursor.execute(
    ...
    def save_files_in_batch(self, files_list, source):
        ...
        data_search_index = [
            (
                remove_accents(item.get('name', '').lower()),
                remove_accents(item.get('description', '').lower()),
                item.get('id'),
                item.get('source')
            ) for item in files_list
        ]
    ...
    def load_files_paged(self, source, page, page_size, search_term=None, sort_by='name_asc', filter_type='all', folder_id=None, advanced_filters=None, explorer_special=False):
        ...
        if search_term:
            norm_search_term = remove_accents(search_term.lower())
            terms, exclude_terms, or_groups, extra_filters = self.parse_search_query(
                norm_search_term)
            valid_terms = [t.strip() for t in (
                or_groups if or_groups else terms) if t and not t.startswith('-') and t.strip() and t.strip().upper() not in ['OR', 'AND']]
            ...
            return self._build_file_objects_from_search(rows)
    ...
    def add_file(self, file_path, source):
        ...
        self.conn.commit()
        self.cursor.execute('INSERT OR REPLACE INTO search_index (name, description, file_id, source) VALUES (?, ?, ?, ?)',
                            (remove_accents(name.lower()), remove_accents(description.lower()), file_id, source))
        self.conn.commit()
