...
class DriveFileGalleryApp(QMainWindow):
    ...
    def debug_search_normalization(self, search_term):
        ...
        # 4. Teste de busca com termo normalizado
        ...
        # 7. Busca direta no FTS5 para debug avançado
        print(f"\n🔧 DEBUG AVANÇADO FTS5:")
        try:
            self.indexer.cursor.execute(
    ...
    def debug_system_status(self):
        """F12 sem termo de busca - Status geral do sistema"""
        print(f"\n" + "="*60)
    ...
    def debug_database_status(self):
        """F11 - Status detalhado do banco de dados"""
        print(f"\n" + "="*60)
        ...
        try:
            ...
            # Amostras de arquivos com acentos no índice
            self.indexer.cursor.execute('''
    ...
    def debug_test_accent_samples(self):
        """F10 - Testar amostras conhecidas com acentos"""
        print(f"\n" + "="*60)
        print(f"🧪 DEBUG TESTE DE AMOSTRAS COM ACENTOS")
        print(f"="*60)

        # Amostras de teste conhecidas
        test_samples = [
            ("formação", "formacao"),
            ("ação", "acao"),
            ("coração", "coracao"),
        ...
        for i, (original, expected_norm) in enumerate(test_samples, 1):
            ...
            try:
                ...
                if has_results and i <= 3:  # Mostrar exemplos para os primeiros 3
