                ...
                for name in dirs:
                    ...
                    try:
                        ...
                        if modified < 0:
                            ...
                            continue
                        if isinstance(modified, (int, float)) and last_sync_datetime:
...
class DriveSyncWorker(QObject):
    ...
    def __init__(self, service, db_name='data/file_index.db'):
        super().__init__()
        self.service = service
        self.db_name = db_name
        self.is_running = True
    ...
    def run(self):
        print("🔄 DriveSyncWorker.run() iniciado")
        ...
        try:
            ...
            for drive_item in processed_items:
                indexer.cursor.execute(
                    ...
                    (drive_item['name'], drive_item['size'])
                )
                local_rows = indexer.cursor.fetchall()
                if local_rows and drive_item['size'] > 0:
                    ...
                    drive_with_size += 1
                    if drive_with_size - matches <= 5:
                        print(
                            f"DEBUG: No match for Drive file: {drive_item['name']} size: {drive_item['size']}")
            print(
                f"Total Drive files: {total_drive}, with size >0: {drive_with_size}, matches: {matches}")
